...
interface_ip_comparisons.py (split out per docs/CODE_AUDIT.md finding F4);
this module handles orchestration - existence checks, physical/L2 property
checks, and categorization.
"""

from .interface_ip_comparisons import (
...
def get_interfaces_needing_config_changes(
    ...
    if "network_resources" in device_facts:
        ...
        if network_resources and isinstance(network_resources, dict):
            ...
            if interfaces_dict and isinstance(interfaces_dict, dict):
                ...
                _debug(
                    f"Loaded {len(facts_by_interface)} interfaces from device facts")
                _debug(
                    f"Sample interface keys: {list(facts_by_interface.keys())[:10]}")
    ...
    for intf_key, intf_data in facts_by_interface.items():
        ...
        if intf_data.get("type") == "lag":
            ...
            if isinstance(lag_members, dict):
                ...
                _debug(
                    f"LAG {lag_name} has members: {list(lag_members.keys())}")

    _debug(
        f"Built LAG membership map with {len(intf_to_lag_map)} member interfaces")
    ...
    for nb_intf in interfaces:
        ...
        # Shallow-copy so this function never mutates the caller's input
        # objects (docs/CODE_AUDIT.md finding F5 / CLAUDE.md 4.3 "filters
        # are pure"). Every `_ip_changes` write below (directly or via
        # populate_ip_changes()/compute_l3_ip_changes()/
        # compute_dhcp_relay_changes()) only ever adds a fresh top-level
        # `_ip_changes` dict, never mutates nested structures shared with
        # the original object, so a shallow copy is sufficient.
        nb_intf = dict(nb_intf)
        ...
        _debug(
            f"Processing interface: {intf_name} (type: {nb_intf.get('type')})")
        ...
        if not device_intf:
            ...
            is_vlan_interface = (
                intf_name.startswith("vlan")
                and get_interface_type_value(nb_intf) == "virtual"
            )
            ...
            if is_vlan_interface:
                ...
                _categorize_interface_for_changes(
                    nb_intf, result, needs_change=True)
                continue
        ...
        # Get interface type
        type_value = get_interface_type_value(nb_intf) or ""
        ...
        is_virtual_interface = get_interface_type_value(nb_intf) == "virtual"
        ...
        if mode_obj and isinstance(mode_obj, dict) and not is_virtual_interface:
            ...
            if nb_mode:
                ...
                if has_vlan_config and device_mode:
                    ...
                    nb_has_tagged_vlans = nb_tagged_vlans and len(
                        nb_tagged_vlans) > 0
        ...
        # IPv4/IPv6/VRF/encapsulation/anycast/link-local comparison is
        # delegated to compute_l3_ip_changes() (interface_ip_comparisons.py)
        # - see docs/CODE_AUDIT.md F4/F5 for why this was split out.
        if nb_intf.get("ip_addresses"):
            enhanced_intf = None
            if enhanced_facts and isinstance(enhanced_facts, dict):
                enhanced_intf = enhanced_facts.get(intf_name)

            l3_needs_change, l3_reasons, l3_ip_changes = compute_l3_ip_changes(
                nb_intf, device_intf, enhanced_intf, intf_name
            )
            if l3_needs_change:
                needs_change = True
                change_reasons.extend(l3_reasons)
            if l3_ip_changes:
                if "_ip_changes" not in nb_intf:
                    nb_intf["_ip_changes"] = {}
                nb_intf["_ip_changes"].update(l3_ip_changes)

        # Check DHCP relay / ip helper-address configuration
        # Delegated to compute_dhcp_relay_changes() (interface_ip_comparisons.py).
        dhcp_needs_change, dhcp_reasons, dhcp_ip_changes = compute_dhcp_relay_changes(
            nb_intf, intf_name, dhcp_relay_facts, ip_helper_addresses
        )
        if dhcp_needs_change:
            needs_change = True
            change_reasons.extend(dhcp_reasons)
        if dhcp_ip_changes:
            if "_ip_changes" not in nb_intf:
                nb_intf["_ip_changes"] = {}
            nb_intf["_ip_changes"].update(dhcp_ip_changes)

        if needs_change:
            _debug(
                f"Interface {intf_name} needs changes: {', '.join(change_reasons)}")
            _categorize_interface_for_changes(
                nb_intf, result, needs_change=True)
        else:
