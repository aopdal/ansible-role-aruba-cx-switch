...
- **`group_interface_ips(interface_ip_list, ospf_facts=None, ospf_process_id=1)`**
    ...
    - `_ip_changes.enabled_change` is deliberately NOT one of these triggers — a VLAN SVI with Active Gateway configured prompts for interactive CLI confirmation on `shutdown`, which `aoscx_config` cannot answer. `tasks/configure_l3_interfaces.yml` reads `_ip_changes.enabled_change` directly instead and pushes it via `aoscx_interface` (REST)
...
- **`build_l3_config_lines(item, interface_type, vrf_type, l3_counters_enable=True, ip_helper_addresses=None)`**
    ...
    - Does NOT emit a `no shutdown`/`shutdown` line for `vlan` or `subinterface` — a VLAN SVI with Active Gateway (anycast) configured prompts for interactive CLI confirmation on `shutdown` (`Continue (y/n)?`), which `aoscx_config` (`network_cli`) cannot answer; the command would silently never take effect. Enabled state for these types is instead pushed via `aoscx_interface` (REST, no CLI prompt) directly by `tasks/configure_l3_interfaces.yml`, reading `_ip_changes.enabled_change`. See the NOTE in `group_interface_ips`'s docstring.
    - For `interface_type` `physical` or `lag`, a `no shutdown`/`shutdown` line IS emitted, but only immediately after `routing` — a deliberate duplicate of what `configure_physical_interfaces.yml`/`configure_lag_interfaces.yml` already pushed earlier in the run, guarding against AOS-CX resetting admin state as a side effect of (re)applying `routing`. This is safe from the same CLI-prompt risk as vlan/subinterface above — Active Gateway is an SVI-only AOS-CX feature. `aoscx_config` only actually sends the line when the device disagrees, so it's a no-op in the common case.
