...
**Cause (fixed in 0.13.20)**: The device-side `vsx_virtual_ip4` REST API attribute (used to detect whether the anycast IP is already configured) can be returned in CIDR form (e.g. `172.18.19.129/27`) on some firmware/API versions, mirroring how `ip4_address` is stored — even though the `active-gateway ip` CLI command itself takes no prefix and NetBox's anycast address is always compared without one. That formatting mismatch made the comparison think the anycast IP was missing on every run. The role now strips any `/prefix` from `vsx_virtual_ip4` before comparing, so this no longer occurs from this cause.

**Second, more common cause (fixed in the Unreleased version above 0.13.20)**: `vsx_virtual_ip4`/`vsx_virtual_ip6` are only *requested* from the REST API — AOS-CX omits attributes that were never requested rather than returning them as `null` — when `custom_fields.device_vsx` is `true` **and** `aoscx_configure_vsx` (or `aoscx_test_mode`) is `true`. `aoscx_configure_vsx` defaults to `false`, and it is a very common setup to leave VSX config-pushing disabled while still configuring L3/anycast interfaces (e.g. running the role with only `aoscx_configure_l3_interfaces: true`, or an automation pipeline that pushes each feature area separately — see `tasks/gather_facts_rest_api.yml`). In that case the anycast comparison received no `vsx_virtual_ip4`/`vsx_virtual_ip6` data at all and always assumed the anycast address was missing. The REST query now also requests these attributes whenever `custom_fields.device_anycast_gateway` is `true` and `aoscx_configure_l3_interfaces` (or `aoscx_test_mode`) is `true`, independent of `aoscx_configure_vsx`/`device_vsx`.

**If you still see this**: enable `-e aoscx_debug=true` and check:

1. The `Interface attributes to query` line under "Debug - Show REST API configuration" — it must include `vsx_virtual_ip4` (and `vsx_virtual_ip6` for IPv6 anycast). If it does not, verify `custom_fields.device_anycast_gateway` is `true` on the device in NetBox (or `custom_fields.device_vsx` + `aoscx_configure_vsx`/`aoscx_test_mode`), and that `aoscx_gather_facts_rest_api: true` is set.
2. The `Anycast comparison for <intf>` debug line — if `device VSX virtual IPv4` doesn't contain the expected address even though it was queried, capture the raw REST response for that interface and compare against `show running-config interface <intf>`.
