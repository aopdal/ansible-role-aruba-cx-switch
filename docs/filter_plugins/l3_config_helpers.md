...
3. **Routing mode, then enabled-state reassertion** (`interface_type` is `'physical'` or `'lag'` only)
   ...
   nature and never emit `routing`; sub-interface parents are handled
   separately in `tasks/configure_physical_interfaces.yml` (not by this
   function).

   The `shutdown`/`no shutdown` line immediately after `routing` is a
   deliberate duplicate of what `configure_physical_interfaces.yml`/
   `configure_lag_interfaces.yml` already pushed earlier in the same run
   (see `tasks/main.yml` ordering — physical/LAG interface config runs
   before L3 config). AOS-CX has been observed to reset a physical/LAG
